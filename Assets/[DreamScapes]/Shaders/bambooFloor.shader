...
/////// Diffuse:
                ...
                float4 node_8492 = tex2D(_bamboo,TRANSFORM_TEX(i.uv0, _bamboo));
                float3 diffuseColor = (sign(node_8492.b)*_colour.rgb);
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
/// Final Color:
                float3 finalColor = diffuse;
                fixed4 finalRGBA = fixed4(finalColor,node_8492.a);
...
/// Final Color:
                float3 finalColor = diffuse;
                fixed4 finalRGBA = fixed4(finalColor * node_8492.a,0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
